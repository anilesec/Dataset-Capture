# https://github.com/vchoutas/torch-mesh-isect/blob/master/mesh_intersection/loss.py
import torch
import pytorch3d
...
def get_collision_idx_batch(faces, n_neighbor=10, bs=10):
    collision_idxs = []
    for _ in range(bs):
        _in = torch.arange(0, faces.max()).repeat(n_neighbor)
        _out = torch.cat([torch.randperm(faces.max()) for _ in range(n_neighbor)])
        collision_idxs.append(torch.stack([_in, _out], 1))
    collision_idxs = torch.stack(collision_idxs)
    return collision_idxs

def get_collision_idxs(faces, n_neighbor=10, type='random'):
    assert type in ['random', 'neighbor']
    # identify neirboors triangle
    collisions = {}
    for x in faces.numpy():
        if type == 'random':
            collisions[x[0]] = torch.randperm(faces.shape[0])[:n_neighbor]
        elif type == 'neighbor':
            if x[0] not in list(collisions.keys()):
                collisions[x[0]] = [x[1], x[2]]
            else:
                for i in [1,2]:
                    if x[i] in list(collisions.keys()) and x[0] in collisions[x[i]]:
                        continue
                    if x[i] not in collisions[x[0]]:
                        collisions[x[0]].append(x[i])
        else:
            raise NameError
    list_collisions = []
    for k, vs in collisions.items():
        for v in vs:
            list_collisions.append([k, v])
    collision_idxs = torch.Tensor(list_collisions).long()
    # print("nb of potential collision to compute: ", collision_idxs.shape[0])
...
class DistanceFieldPenetrationLoss(nn.Module):
    ...
    def forward(self, triangles, collision_idxs):
            ...
            coll_idxs = torch.nonzero(collision_idxs[:, :, 0].ge(0))
            ...
            recv_triangles = triangles[batch_idxs, receiver_faces] # (BxCxNx3)
            intr_triangles = triangles[batch_idxs, intruder_faces] # (BxCxNx3)
