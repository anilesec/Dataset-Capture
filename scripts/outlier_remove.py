...
def rm_outlier(inp_pcds_dir, seq_dir, vox_size, radius, num_nibrs, down_sample=True):
    ...
    for pcdp in tqdm(pcds_pths):
        ...
        cl, ind = voxel_down_pcd.remove_statistical_outlier(nb_neighbors=200, std_ratio=1.5)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_nibrs', type=int, default=250,
                        help='min no of points to be present in the sphere for inlier cond.')
    
    args = parser.parse_args()
    print("args:", args)   
    
    rm_outlier(args.inp_pcds_dir, args.seq_dir, args.vox_size, args.radius, args.num_nibrs, down_sample=True)
