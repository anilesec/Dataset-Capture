...
if __name__ == "__main__":
    ...
    all_minhand_jts3d_txts = sorted(glob.glob(osp.join(RES_DIR, '*/jts3d_min-hand/*.txt')))
    ...
    all_MPJPE_PA = []
    missing_poses_frms = []
    for txtp in tqdm(all_minhand_jts3d_txts):
        # check if GT 3D jts exist for corresponding det
        jts3d_ann_pth = pathlib.Path(txtp.replace('jts3d_min-hand', 'jts3d'))
        if not jts3d_ann_pth.exists():
            print(f'Missing: {jts3d_ann_pth}')
            missing_poses_frms.append(jts3d_ann_pth)
            continue
        jts3d_minhand = np.loadtxt(txtp)
        # all_jts3d_minhand.append(jts3d_minhand)
        ...
        MPJPE, MPJPE_PA, _, _ = compute_errors((jts3d_minhand * 0.075).reshape(1, 21, 3), jts3d_ann[ours2mano()].reshape(1, 21, 3))

        jts_img = viz_hand_jts3d(hand_jts=jts3d_minhand, jts_order='CP', grid_axis='ON',
                                  line_sz=2, dot_sz=4, elev=-90, azim=-90,
                                  xlim=[jts3d_minhand[:, 0].min(), jts3d_minhand[:, 0].max()],
                                  ylim=[jts3d_minhand[:, 1].min(), jts3d_minhand[:, 1].max()],
                                  zlim=[jts3d_minhand[:, 2].min(), jts3d_minhand[:, 2].max()],
                                  resln=None, title='3D Joints min-hand')

        gt_img = viz_hand_jts3d(hand_jts=jts3d_ann, jts_order='OURS', grid_axis='ON',
                                  line_sz=2, dot_sz=4, elev=-90, azim=-90,
                                  xlim=[jts3d_ann[:, 0].min(), jts3d_ann[:, 0].max()],
                                  ylim=[jts3d_ann[:, 1].min(), jts3d_ann[:, 1].max()],
                                  zlim=[jts3d_ann[:, 2].min(), jts3d_ann[:, 2].max()],
                                  resln=None, title='GT 3D Joints')
        
        plt.imsave('./tmp.png', jts_img)
        plt.imsave('./gt.png', gt_img)
        ...
        # bb()
        # img_minhand = viz_hand_jts3d(jts3d_minhand, 'CP', grid_axis='ON',
        #             line_sz=2, dot_sz=4, elev=-90, azim=-90,
        #             xlim=[jts3d_minhand[:, 0].min() - 0.1, jts3d_minhand[:, 0].max() + 0.1],
        #             ylim=[jts3d_minhand[:, 1].min() - 0.1, jts3d_minhand[:, 1].max() + 0.1],
        #             zlim=[jts3d_minhand[:, 2].min() - 0.1, jts3d_minhand[:, 2].max() + 0.1],
        ...
        #             title='3D Joints in wrist frame')
        # plt.imsave('./minhand.png', img_minhand)
