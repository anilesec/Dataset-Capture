...
if __name__ == "__main__":
    ...
    for sqn in tqdm(all_sqns):
        ...
        # bb()
        # ref relative rot (this is not correct because it gives 
        # higher error but I do not know why)
        # all_rel_rot_clmp_inv = np.array(
        #     [np.linalg.inv(cps_clmp[idx, :3, :3]) @ cps_clmp[0, :3, :3]
        #     for idx in range(1, len(cps_clmp))]
        #     )
        # all_rel_rot_ann_inv = np.array(
        #     [np.linalg.inv(cps_ann[idx, :3, :3]) @ cps_ann[0, :3, :3]
        #     for idx in range(1, len(cps_ann))]
        #     )
        # # rel-rel rot error
        # all_rel_rot_err_inv = np.array(
        # [(geodesic_distance_for_rotations(all_rel_rot_clmp_inv[i, :3, :3], all_rel_rot_ann_inv[i, :3, :3]) * (180 / np.pi))
        # for i in range(1, len(all_rel_rot_clmp_inv))]
        # )
        # disp_rot_err(all_rel_rot_err_inv)
        ...
        save_dict_pth = osp.join(RES_DIR, sqn, 'eval_rel_pose_clmp_all.pkl')
