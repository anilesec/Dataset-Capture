...
def calc_3d_metric_VH(rec_meshfile, gt_meshfile, align=False, sampl_num=100000, dist_thresh=None, save_pkl=None, save_seq_dir=None):
    """
    ...
    if save_pkl:    
        pkl_save_pth = osp.join(save_seq_dir, f"vh_recon_eval_sample{sampl_num}_dth_{dist_thresh:.4f}.pkl")
        os.makedirs(osp.dirname(pkl_save_pth),exist_ok=True)
        # pkl_save_pth = osp.join(osp.dirname(osp.dirname(rec_meshfile)), f"vh_recon_eval_sample{sampl_num}_dth_{dist_thresh:.4f}.pkl") 
...
if __name__ == '__main__':
    ...
    for sqn in tqdm(all_sqn_ids):
        ...
        rec_mesh_pth = pathlib.Path(glob.glob(osp.join(VH_RECON_RES_DIR, sqn, 'VH/*.ply'))[0])
        ...
        print(f'rec_mesh_pth: {str(rec_mesh_pth)}')
        print(f'gt_mesh_pth: {str(gt_mesh_pth)}')
