...
if __name__ == "__main__":
    ...
    all_fmocap_jts3d_pkls = sorted(glob.glob(osp.join(FMOCAP_RES_DIR, '*/mocap/*.pkl')))
    # bb()
    all_jts3d_ann = []
    all_jts3d_fmocap = []
    all_MPJPE = []
    ...
    missing_poses_frms = []
    missing_dets = []
    for pklp in tqdm(all_fmocap_jts3d_pkls):
        sqn = pklp.split('/')[-3]
        filename = osp.basename(pklp).split('_')[0]
        # check if GT 3D jts exist for corresponding hocnet det
        jts3d_ann_pth = pathlib.Path(osp.join(RES_DIR, sqn, f'jts3d/{filename}.txt'))
        if not jts3d_ann_pth.exists():
            print(f'Missing: {jts3d_ann_pth}')
            missing_poses_frms.append(jts3d_ann_pth)
            continue
        jts3d_fmocap = load_pickle_jts3d(pklp)
        if jts3d_fmocap is None:
            missing_dets.append(filename)
            continue
        all_jts3d_fmocap.append(jts3d_fmocap)

        jts3d_ann = np.loadtxt(jts3d_ann_pth)
        all_jts3d_ann.append(all_jts3d_ann)
        # bb()
        MPJPE, MPJPE_PA, _, _ = compute_errors(jts3d_fmocap.reshape(1, 21, 3), jts3d_ann[ours2hocnet()].reshape(1, 21, 3))
