...
if __name__ == "__main__":
    ...
    for imgp in tqdm(all_rgbs_pths):
        jts3d_minhand_pth = pathlib.Path(imgp.replace('rgb', 'jts3d_min-hand').replace('png', 'txt'))
        jts3d_ann_pth = pathlib.Path(imgp.replace('rgb', 'jts3d').replace('png', 'txt'))
        ...
        if not jts3d_minhand_pth.exists():
            print(f'Missing minhand dets: {jts3d_minhand_pth}')
            missing_minhand_dets.append(jts3d_minhand_pth)
            all_MPJPE.append(np.inf)
            all_MPJPE_PA.append(np.inf)
            all_pck.append(np.full((21, 1), np.inf))
        else:
            jts3d_minhand = np.loadtxt(jts3d_minhand_pth)
            ...
            # bb()
            jts3d_minhand[:, 0] *= -1 
            bone_length = 0.075 # 75mm from issues of min-hand repo

            MPJPE, MPJPE_PA, errors_pck, _ = compute_errors((jts3d_minhand * bone_length).reshape(1, 21, 3), jts3d_ann[ours2mano()].reshape(1, 21, 3))
            all_MPJPE.append(MPJPE)
            all_MPJPE_PA.append(MPJPE_PA)
            # print(errors_pck.shape)
            all_pck.append(errors_pck.reshape(21, 1))

    # all_jts3d_minhand = np.array(all_jts3d_minhand)
    ...
    print(f"missing_minhand_dets: Total: {len(missing_minhand_dets)}")

    # bb()
    # pck value
    minhand_pck_dict = dict()
    for pck_th in np.arange(0.001, 0.101, 0.001):
        pck_final = compute_pck(all_pck, pck_th) * 100.
        minhand_pck_dict[f'{pck_th:.4f}'] = pck_final
        print(f"PCK with thresh {pck_th:.4f}: {pck_final:.4f} %")   

    # plot pck
    import matplotlib.pyplot as plt
    x_axis = [float(k) for k in minhand_pck_dict.keys()]
    y_axis = [int(v) for v in minhand_pck_dict.values()]
    plt.figure()
    plt.plot(x_axis, y_axis, '-r')
    plt.xlabel('')
    plt.ylabel('')
    plt.savefig(osp.join(RES_DIR, 'minhand_pck.png'))
    print('Done!!')